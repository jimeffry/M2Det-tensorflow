...
class M2Det:
    def __init__(self, inputs, is_training, use_sfam=False):
        ...
        self.levels = 8
        self.scales = 6
        self.num_priors = cfgs.Num_Priors # 2 scales and 3 ratios
        self.build(inputs, is_training)

    def build(self, inputs, is_training):
        with tf.variable_scope('VGG16'):
            net = inputs
            net = vgg_layer(net, is_training, 64, 2,name=1)
            net = vgg_layer(net, is_training, 128, 2,name=2)
            net = vgg_layer(net, is_training, 256, 3,name=3)
            net = vgg_layer(net, is_training, 512, 3, pooling=False,name=4)
            feature1 = net
            net = vgg_layer(net, is_training, 1024, 3,name=5)
            feature2 = net

        with tf.variable_scope('M2Det'):
            with tf.variable_scope("FFMS"):
            ...
            with tf.variable_scope("SFAM"):
                ...
                for i in range(self.scales):
                    ...
                    if self.use_sfam:
                        with tf.variable_scope('SFAM_{}'.format(i)):
            ...
            with tf.variable_scope('prediction'):
                ...
                for i, feature in enumerate(features):
                    ...
                    with tf.variable_scope("Feature_{}".format(i)):
                        ...
                        all_reg.append(reg)
                with tf.variable_scope("Features_pred"):
                    with tf.variable_scope("bbox_reg"):
