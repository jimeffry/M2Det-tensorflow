...
def vgg_layer(x, is_training, filters, num_blocks, pooling=True,name=1):
    with tf.variable_scope("Block_{}".format(name)):
        for idx in range(num_blocks):
            with tf.variable_scope('Conv_Block_{}'.format(idx)):
...
def tum(x, is_training, scales):
    ...
    for i in range(scales - 1):
        with tf.variable_scope("Conv_infer_{}".format(i)):
    ...
    for i in range(1, scales):
        with tf.variable_scope("Conv_transform_{}".format(i)):
    ...
    for i in range(scales):
        with tf.variable_scope("Tum_out_{}".format(i)):
